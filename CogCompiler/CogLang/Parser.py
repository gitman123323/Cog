...
class Parser:
    ...
    def __parse_let_statement(self) -> LetStatement:
        ...
        # Current token is LET, so check if next token is MUT
        if self.__peek_token_is(TokenType.MUT):
            self.__next_token()                 # consume 'mut'
    ...
    def __parse_postfix_expression(self, left_node: Expression) -> AssignStatement:
        # Transform: a++   →   a = a + 1
        if isinstance(left_node, IdentifierLiteral):
            assign = AssignStatement()
            assign.ident = left_node
            assign.operator = "="
            assign.right_value = InfixExpression(
                left_node=left_node,
                operator="+",
                right_node=IntegerLiteral(value=1)
            )
            return assign
        else:
            self.errors.append("++ can only be applied to identifiers.")
            return None
